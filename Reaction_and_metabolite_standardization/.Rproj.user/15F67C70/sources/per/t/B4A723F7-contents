...
gene_all <- vector()
rxn_all <- vector()
strain <- list.files('data/strain specific model from RAVEN_kegg')

for (i in strain) {
  print(i)
  #i <- "biocyc_panYeast2_45_100"
  inputfile <- paste('data/strain specific model from RAVEN_kegg/',i, '/excelGenes.txt', sep = "")
  inputfile2 <- paste('data/strain specific model from RAVEN_kegg/',i, '/excelRxns.txt', sep = "")
