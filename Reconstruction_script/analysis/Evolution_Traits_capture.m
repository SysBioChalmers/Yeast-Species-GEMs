...
% Figure S2d and also the supplementary table for the ura1 hiroizonal gene
% transfer
%% load data
currentpath = pwd;
...
cd(currentpath)
uratable(:,2:4) = num2cell(ura);
uratable(:,1) = strainlist;
uratable = cell2table(uratable);
uratable.Properties.VariableNames = {'species','original','both','gene transfer'};
writetable(uratable,'URA1HGT_supp.txt','Delimiter', '\t','QuoteStrings',false);
