current = pwd;
%Rescale biomass composition, fit sigma for growth and fix EtOH production
...
misspredicted = zeros(length(Fchange),1);
misspredicted(find(Fchange<0.9)) = 1;
misspredicted(find(Fchange>1.1)) = 2;
...
cd(current)
curate_ecModels('ecModel_batch_curated','ecModel_batch_curated_yield','ecModels_metrics_curated_yield',1,true,true)
% 
